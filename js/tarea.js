class Tarea {
  ...
  borrarTarea() {
    fetch("https://api-todo-poml.onrender.com/api-todo/borrar/" + this.id, {
      method: "DELETE",
    })
  ...
  toggleEstado() {
    return fetch(`https://api-todo-poml.onrender.com/api-todo/actualizar/${this.id}/2`, {
      method: "PUT",
    }).then((respuesta) => respuesta.json());
    ...
    if (this.editando) {
      ...
      if (
        ...
        let { resultado } = await fetch(
          `https://api-todo-poml.onrender.com/api-todo/actualizar/${this.id}/1`,
          {
            method: "PUT",
